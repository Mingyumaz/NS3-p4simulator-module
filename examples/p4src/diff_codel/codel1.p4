...
#define SOJOURN_TARGET_2 500
#define SOJOURN_TARGET_1 2 * SOJOURN_TARGET_2
#define SOJOURN_TARGET_0 3 * SOJOURN_TARGET_2
#define SOJOURN_TARGET_x SOJOURN_TARGET_2

#define CONTROL_INTERVAL_0 48w10000 // 10ms
#define CONTROL_INTERVAL_1 48w10000 // 10ms
#define CONTROL_INTERVAL_2 48w10000 // 10ms
#define CONTROL_INTERVAL_x 100000 // 100ms  

// #define SOJOURN_TARGET 5000 // normally should be 5ms
...
register<bit<2>>(NUM_QUEUE) r_slice_drop; // queue_length_limit_pre_drop // 时间可以设定为codel的一个轮回时间
...
control MyIngress(inout headers hdr,
    ...
    action set_priority(bit<7> value) {
        if (meta.flowi.is_tcp == 1w1) {
            ...
            if (value != 7w0){
                // udp with a lower priority
                standard_metadata.priority = (bit<3>)value - 3w1;
            }
            else {
                standard_metadata.priority = (bit<3>)value;
            }
    ...
    apply {
        if (hdr.ipv4.isValid() && hdr.ipv4.ttl > 8w0 || hdr.arp.isValid()) {
            ...
            bit<2> rand_val;
            random<bit<2>>(rand_val, 0, 3); // value 0,1,2,3
            if (meta.prio.prio_drop != 2w0) {
                if (rand_val < meta.prio.prio_drop) {
                    // meta.prio.prio_drop=1 -> 25%drop
                    // meta.prio.prio_drop=2 -> 50%drop
                    // meta.prio.prio_drop=3 -> 75%drop
                    meta.ns3i.ns3_drop = 1; // connect ns-3 --> drop @ns-3 
                    mark_to_drop(standard_metadata); // queue length --> drop @bmv2
                    r_slice_drop.write((bit<32>)meta.codel.queue_id, (bit<2>)0); // no drop
                }
                if (meta.prio.prio_drop == 2w1) {
                    r_slice_drop.write((bit<32>)meta.codel.queue_id, (bit<2>)0); // no drop
                }
            }
...
control MyEgress(inout headers hdr,
    ...
    apply {
        ...
        if (standard_metadata.deq_qdepth > current_queue_threshold_min + 10){
            r_slice_drop.write((bit<32>)meta.codel.queue_id, (bit<2>)1); // drop with threshold(min)
        }
        else if (standard_metadata.deq_qdepth > current_queue_threshold_max + 10){
            r_slice_drop.write((bit<32>)meta.codel.queue_id, (bit<2>)3); // drop with threshold(max)
        }
        else{
            r_slice_drop.write((bit<32>)meta.codel.queue_id, (bit<2>)0); // no drop
        }
