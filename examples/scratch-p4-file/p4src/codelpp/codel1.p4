...
#define SOJOURN_TARGET 5000 // 5ms
#define CONTROL_INTERVAL 48w100000 
    ...
    apply {
        if (hdr.ipv4.isValid() && hdr.ipv4.ttl > 8w0 || hdr.arp.isValid()) {
            ...
            if (standard_metadata.priority == 3w7) {
                r_slice_drop_7.read(meta.prio.prio_drop, (bit<32>)meta.codel.queue_id);
            } else if (standard_metadata.priority == 3w6) {
                r_slice_drop_6.read(meta.prio.prio_drop, (bit<32>)meta.codel.queue_id);
            } else if (standard_metadata.priority == 3w5) {
                r_slice_drop_5.read(meta.prio.prio_drop, (bit<32>)meta.codel.queue_id);
            } else if (standard_metadata.priority == 3w4) {
                r_slice_drop_4.read(meta.prio.prio_drop, (bit<32>)meta.codel.queue_id);
            } else if (standard_metadata.priority == 3w3) {
                r_slice_drop_3.read(meta.prio.prio_drop, (bit<32>)meta.codel.queue_id);
            } else if (standard_metadata.priority == 3w2) {
                r_slice_drop_2.read(meta.prio.prio_drop, (bit<32>)meta.codel.queue_id);
            } else if (standard_metadata.priority == 3w1) {
                r_slice_drop_1.read(meta.prio.prio_drop, (bit<32>)meta.codel.queue_id);
            } else if (standard_metadata.priority == 3w0) {
                r_slice_drop_0.read(meta.prio.prio_drop, (bit<32>)meta.codel.queue_id);
            }
...
control MyEgress(inout headers hdr,
    ...
    apply {
        //meta.codel.queue_id = standard_metadata.egress_port; // put directly

        // different priority -> different queue length limit
        if (standard_metadata.priority == 3w7) {
            ...
            if (standard_metadata.deq_qdepth > 19w700) {// 队列长度 大于 700
            ...
            if (standard_metadata.deq_qdepth > 19w450) {// 队列长度 大于 450
            ...
            r_slice_time_4.write((bit<32>)meta.codel.queue_id, (bit<32>)standard_metadata.deq_timedelta);
            r_slice_enq_4.write((bit<32>)meta.codel.queue_id, (bit<19>)standard_metadata.enq_qdepth);
            r_slice_deq_4.write((bit<32>)meta.codel.queue_id, (bit<19>)standard_metadata.deq_qdepth);
