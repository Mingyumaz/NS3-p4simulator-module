...
struct flowtype_t {
    bit<1>     is_tcp;     // weather if the packst is tcp 1 or not 0(udp)
}
...
control MyIngress(inout headers hdr,
    ...
    apply {
        if (hdr.ipv4.isValid() && hdr.ipv4.ttl > 8w0 || hdr.arp.isValid()) {
            ...
            if (meta.flowtype.is_tcp == 1) {
                meta.ns3i.ns3_drop = 1; 
                mark_to_drop(standard_metadata); // drop all the tcp flows
            }
