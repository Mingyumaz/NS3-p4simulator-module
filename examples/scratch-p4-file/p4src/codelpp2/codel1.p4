...
#define CONTROL_INTERVAL 48w10000 // 10ms
        ...
        if (hdr.ipv4.isValid() && hdr.ipv4.ttl > 8w0 || hdr.arp.isValid()) {
            ...
            if (meta.prio.prio_drop != 2w0) {
                if (rand_val < meta.prio.prio_drop) {
                    r_slice_drop.write((bit<32>)meta.codel.queue_id, (bit<2>)0); // only one fast drop
...
control c_codel(inout headers hdr, 
    ...
    action a_codel_control_law(bit<48> value) {
        ...
        r_codel_next_drop.write((bit<32>)meta.codel.queue_id, (bit<48>)meta.codel.drop_next); // 对应queue_id的drop_next保存至寄存器。
    }
    ...
    action a_go_to_drop_state() {
        meta.ns3i.ns3_drop = 1;      // add for connect ns-3 --> drop @ns3 
	    mark_to_drop(standard_metadata);
        r_codel_state_dropping.write((bit<32>)meta.codel.queue_id, (bit<1>)1); //给对应queue_id的队列进入drop阶段。// 1 就是在drop state, 0 表示未在drop阶段。
    ...
    apply {
        // 1. CoDel initlization
        a_codel_init();
    
        // 2. CoDel measurements
        ...
        // 3. CoDel measurement processing
...
control MyEgress(inout headers hdr,
    ...
    apply {
        ...
        if (current_queue_threshold_min != 0){
            if (current_queue_threshold_max != 0){
                if (standard_metadata.enq_qdepth > current_queue_threshold_min + 1){
                r_slice_drop.write((bit<32>)meta.codel.queue_id, (bit<2>)1); // drop with min
                }
                else if (standard_metadata.enq_qdepth > current_queue_threshold_max + 1){
                    r_slice_drop.write((bit<32>)meta.codel.queue_id, (bit<2>)2); // drop with max
                }
            }
        }
