...
#define SOJOURN_TARGET_2 5000
#define SOJOURN_TARGET_1 SOJOURN_TARGET_2
#define SOJOURN_TARGET_0 SOJOURN_TARGET_2
#define SOJOURN_TARGET_x SOJOURN_TARGET_2

#define CONTROL_INTERVAL_0 48w100000 // 10ms
#define CONTROL_INTERVAL_1 48w100000 // 10ms
#define CONTROL_INTERVAL_2 48w100000 // 10ms
...
control c_codel(inout headers hdr, 
    ...
    apply {
        ...
        if (standard_metadata.deq_timedelta < sojourn_target ) { //|| standard_metadata.enq_qdepth < 19w1
...
control MyEgress(inout headers hdr,
    ...
    apply {
        ...
        if (standard_metadata.priority != 3w7) {
            ...
            if (standard_metadata.enq_qdepth > current_queue_threshold_min){
                r_slice_drop.write((bit<32>)meta.codel.queue_id, (bit<2>)1); // drop with threshold(min)
            }
            else if (standard_metadata.enq_qdepth > current_queue_threshold_max){
